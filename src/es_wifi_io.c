...
// While main.cpp call high-levl fucntions, Wifi module requires a protocol to shift bits over an internal Serial Peripher Interface communication bus.
...
//MspInit - MCU Support Package Initialization.
// To turn on peripheral clocks and configure physical GPIO routing.
...
int8_t SPI_WIFI_Init(void)
{
  ...
  hspi.Init.Mode              = SPI_MODE_MASTER;  //sets STM32 as master
  ...
  HAL_GPIO_WritePin(GPIOH, GPIO_PIN_1, GPIO_PIN_RESET); //clears wifi chip's registers by pulling physical reset to ground for 10ms
  ...
  HAL_GPIO_WritePin(GPIOH, GPIO_PIN_1, GPIO_PIN_SET); //resets high
...
//Send and revieve wrap SPI transmission to move payloads like our formatted string packets
