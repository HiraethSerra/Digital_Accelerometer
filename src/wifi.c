...
//Hooks high level wifi layer to low level SPI layer
...
WIFI_Status_t WIFI_OpenClientConnection(uint32_t socket, WIFI_Protocol_t type,
    ...
    conn.Number = socket; //0
    conn.RemotePort = port; //5000
    conn.LocalPort = local_port;
    conn.Type = (type == WIFI_TCP_PROTOCOL) ? ES_WIFI_TCP_CONNECTION
                                            : ES_WIFI_UDP_CONNECTION;
    conn.RemoteIP[0] = location[0]; //172
    conn.RemoteIP[1] = location[1]; //20
    conn.RemoteIP[2] = location[2]; //10
    conn.RemoteIP[3] = location[3]; //2
...
// Takes raw integers, creates a structured object conn that has target IP, listening port and UDP mode
...
//Every time BMA180 triggers, main loop passes data string to this function.
//It passes playload array to driver.
